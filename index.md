...
live database, runs 68 rules over every policy, and flags the
...
**Z3 SMT solver** to *prove* the leak (SEC038). 19 of the 68 rules
...
- **[Rule reference (AGENTS.md)](https://github.com/pgrls/pgrls/blob/main/AGENTS.md)** —
  all 68 rules, each with detection logic, severity, and remediation.
...
- **[Generate — scaffold correct RLS](generate/)** — `pgrls generate` writes
  the policies you don't have, output designed to lint clean; `--strict-binding`
  makes a forgotten tenant fail loudly instead of returning an empty result.
